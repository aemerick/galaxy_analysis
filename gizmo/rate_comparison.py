...
# abundance tables
#
def load_with_FIRE(sim_index, wdir, return_model = False):
    """
    ...
    part = ga.io.Read.read_snapshots(['gas','stars'],
                              'index',
                              sim_index,
...
if __name__ == "__main__":
    ...
    for runname in runs.keys():
        ...
        all_data[runname] = {}
        all_data[runname] = compute_all_errors(part,'star')
    ...
    for e in elements:
        for runname in runs.keys():
            ...
            ax.plot(all_data[runname]['cbins'], np.cumsum(all_data[runname]['hist']) / (1.0*np.sum(all_data[runname]['hist'])),
